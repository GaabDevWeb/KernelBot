...
from kernel.structured_log import ACL_MOD_DATABASE, log_event, redact_secrets
...
def fetch_db_document_meta(settings: Settings) -> list[dict[str, object]]:
    """Lista metadados de documentos activos (sem `content`) para Ops / Documentos.

    Cada item: discipline, slug, title, order, updated_at (ISO ou None), source.
    Devolve [] se DB indisponível — o caller pode cair no índice RAM.
    """
    if not all([settings.db_host, settings.db_name, settings.db_user]):
        return []
    ...
    try:
        ...
        with conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT discipline, slug, title, `order`, updated_at "
                    "FROM knowledge WHERE active = 1 "
                    "ORDER BY discipline, `order`, slug"
                )
                rows = cursor.fetchall()
        ...
        for row in rows:
            ...
            if updated is not None and hasattr(updated, "isoformat"):
                updated_s: str | None = updated.isoformat(sep=" ", timespec="seconds")
        ...
        if getattr(e, "args", None) and e.args and e.args[0] == 2003:
            log_event(
                ...
                logging.WARNING,
                ACL_MOD_DATABASE,
                "document_meta_unreachable",
                "MySQL inacessivel ao listar documentos",
            ...
            log_event(
                ...
                ACL_MOD_DATABASE,
                "document_meta_error",
                "falha ao listar metadados knowledge",
                metadata={
                    "host": settings.db_host,
                    "port": settings.db_port,
                    **_db_error_metadata(e),
