...
continua em `kernel.rag.search`); ele normaliza termos, aplica regras de
...
- `index_gap` em `DecisionReason` apenas para contrato tipado e ACL_META; a
  decisão é emitida em `kernel.orchestrator.context` (catálogo confiante + chave fora do
...
from kernel.structured_log import ACL_MOD_DECISION, log_event
