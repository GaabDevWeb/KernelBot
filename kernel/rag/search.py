...
`kernel.rag.retrieval.build_decision`. O plano
...
from kernel.config import GlobalContextMode
from kernel.config import Settings
from kernel.structured_log import ACL_MOD_SEARCH, log_event
from kernel.knowledge.database import fetch_db_chunks, fetch_db_discipline_ids
from kernel.rag.retrieval import CANDIDATE_K, RetrievalCandidate, expand_query_tokens
...
class SearchEngine:
    ...
    def search_candidates(
        ...
        with self._lock:
            ...
            # Disciplina pedida mas inválida/desconhecida → não vazar merge global.
            if raw_filter and discipline_fail_closed():
                log_event(
                    log,
                    logging.WARNING,
                    ACL_MOD_SEARCH,
                    "discipline_fail_closed",
                    "disciplina inválida; sem merge global",
                    metadata={"raw": raw_filter},
                )
                _log_search_candidates(query, None, [], candidate_k)
                return []
